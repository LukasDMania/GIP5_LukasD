...
public class StockAnalyticsView extends AppLayoutTemplate implements ViewContractLD {
    ...
    private HorizontalLayout buildKpiRow() {
        ...
        row.add(
                createKpiCard("Total Stock Adjustments", stockAdjustmentService.getTotalStockAdjustments()),
    ...
    private VerticalLayout lowStockProductsGrid() {
        ...
        container.add(new H3("Low Stock Products"));
    ...
    private JFreeChartComponent createAdjustmentsPerProductBarChart() {
    ...
    private JFreeChartComponent createProductStockVsTotalDeltaScatterPlot() {
        ...
        totalDeltaPerProduct.forEach((product, delta) -> series.add(product.getStock(), delta));
    ...
    private JFreeChartComponent createStockoutsOverTimeAreaChart() {
        ...
        JFreeChart chart = ChartUtil.createAreaChart(
                "Daily Stockouts Over Time", "Date", "Number of Stockouts", dataset
        );
